...
public class Scene2 extends Scene {
	...
	public void init(Window window) {
		...
		while(nextDegree <= 360) {
			...
			protagCoords.add(new Vec4(Math.cos(Math.toRadians(currentDegree))/2, Math.sin(Math.toRadians(currentDegree))/2, -10, 1));
			protagCoords.add(new Vec4(Math.cos(Math.toRadians(nextDegree))/2, Math.sin(Math.toRadians(nextDegree))/2, -10, 1));
	...
	public void renderFrame(Window window, float musicTime) {
		...
		if(protagCoords.get(1).x < Math.cos(Math.toRadians(currentDegree))) {
			for(int i = 0; i < protagCoords.size(); i++ ) {
				float xTemp = protagCoords.get(i).x;
				float yTemp = protagCoords.get(i).y;
				protagCoords.set(i, new Vec4(xTemp * 1.0006f, yTemp * 1.0006f, -10, 1));
				
			}
			radius = radius * 1.0006f;
			glUniform1f(shaders.getUniformLocation("radius"), radius);
		}
