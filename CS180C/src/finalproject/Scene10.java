...
public class Scene10 extends Scene {
	...
	private String fragShaderFile = "src/finalproject/scene10.fragment.glsl";
	
	private Mat4 projectionMatrix;
	private Mat4 modelViewMatrix;
	
	public Scene10(float startTime) {
		super(startTime);
	...
	public void init(Window window) {
		...
		while(nextDegree <= 360) {
			...
			protagCoords.add(new Vec4(Math.cos(Math.toRadians(currentDegree))/2, Math.sin(Math.toRadians(currentDegree))/2, -5, 1));
			protagCoords.add(new Vec4(Math.cos(Math.toRadians(nextDegree))/2, Math.sin(Math.toRadians(nextDegree))/2, -5, 1));
